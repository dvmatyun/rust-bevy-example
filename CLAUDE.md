...
> **Rust standards:** [.claude/skills/rust/SKILL.md](.claude/skills/rust/SKILL.md) — invoke when writing or reviewing any Rust code (ownership, errors, async, anti-patterns).
...
- [x] Valheim-style terrain mesh — replaced the per-cell voxel cubes with a single flat-shaded heightmap mesh built from `TerrainHeights` (one entity, ~24K verts vs 4K entities). Vertex colours per biome on a single white StandardMaterial. Mesh rebuilds on `Changed<TerrainHeights>` so future destruction is a one-line addition.
- [x] Multi-storey buildings — `Building { footprint: Rect, floor_heights: Vec<f32> }` component. `snap_to_ground` now picks the highest walkable surface (terrain OR any `Building` floor) at-or-below the player's current Y. Demo: 3-storey lodge near (5, 5). Press T (Shift+T) to teleport up (down) 3 units between floors — proper climbing needs a physics engine (`bevy_rapier3d` / `avian3d`); planned in [docsai/world-and-graphics-plan.md](docsai/world-and-graphics-plan.md).
